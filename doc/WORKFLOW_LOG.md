...
- Actor: Codex.
- Intent: allow a venue schedule such as opening at 3:00 PM and closing at 2:00 AM the following calendar day, without rejecting it as invalid.
- Changed: overnight-aware `VenueSettings` duration and hour list; default mock schedule set to 3:00 PM to 2:00 AM; admin day availability, quick/fixed booking timestamps, and occupancy calculations now use an operational day rather than a simple calendar-day comparison; Supabase migration `20260921140000_allow_overnight_venue_hours.sql`; focused settings tests.
...
- Actor: Codex.
- Intent: make the group-invitation URL testable by a teammate in a fresh browser tab instead of only inside the organizer's active mock session.
...
- Source control: Docker deployment commit `dac5859` pushed to `origin/main`.
...
- Actor: Codex.
- Intent: initialize the approved Supabase client safely without persisting public runtime values in source control or replacing unverified mock repositories.
...
- Web preview: `flutter build web` passed and the local build is served at `http://127.0.0.1:7366/` for this desktop session.
- Source control: remediation commit `be4bef5` pushed to `origin/main`.
- Next action: perform browser acceptance checks for the fixed booking and validation paths before backend work is approved.
...
### 2026-09-08 17:22 | Player Slot Booking Icon | Status: completed

- Actor: Codex.
- Intent: give each player-facing available slot the same compact action affordance used by the admin schedule.
- Changed: `lib/features/slots/presentation/available_slots_screen.dart`; `test/widget_test.dart`; `doc/WORKFLOW_LOG.md`.
